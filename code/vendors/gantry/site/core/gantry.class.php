		...
		if (empty($this->_features))
		{
				...
				if (file_exists($feature_path) && is_dir($feature_path)){
					...
					while (false !== ($entry = $d->read())) {
						if($entry != '.' && $entry != '..'){
							...
							if (!isset($raw_features[$feature_name])) {
								$raw_features[$feature_name] = $feature_path.DS.$feature_name.'.php';
							}
						}
					}
					$d->close();
			...
			foreach ($ordered_features as $ordered_feature) {
				if (array_key_exists($ordered_feature, $raw_features)){
					$this->_features[$ordered_feature] = $raw_features[$ordered_feature];
				}
			}
			foreach ($raw_features as $feature => $path){
				if (!isset($this->_features[$feature])){
					$this->_features[$feature] = $path;
				}
			}
		}
		
		foreach ($this->_features as $feature => $path) {
			$className = 'GantryFeature'.ucfirst($feature);
