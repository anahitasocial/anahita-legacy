...
<div class="an-entities an-apps">
	...
	<div class="an-entity an-app" scroll-handle="<?=$app->id?>">
		<div class="app-actions">
			<?php if ( $app->authorize('install', array('actor'=>$entity)) ) : ?>
			<a class="btn btn-primary" data-trigger="Submit" href="<?=@route($entity->getURL().'&action=addapp&app='.$app->component)?>">
				<?= @text('COM-ACTORS-APP-ACTION-INSTALL') ?>
			</a>						
			<?php else : ?>
			<a class="btn" data-trigger="Submit" href="<?=@route($entity->getURL().'&action=removeapp&app='.$app->component)?>">
			    ...
			    <form>	
				    <div class="control-label">
	    			    ...
	    			    <?php foreach($app->getFeatures() as $feature) : ?>
	    			        ...
	    			        <?php $enabled = !$app->authorize('install', array('actor'=>$entity)) ?>
